...
### ARC-P13 — 최종 일관성·투고 파일
- **chokepoint group 명시(1)**: `CH = 4 NSR 해협 + Bering–Chukchi = 5 지역 / 160 main-season units`를
  §2.1·Table 1 각주·Table 3/3b 캡션·§4.1 첫 등장 지점에 못박았다. 모든 chokepoint 수치를 이 그룹으로 재계산해
  확인: **160 units, 45/45/25(post = 2025 15 + 2026 10, 2026 freeze-up은 기록 밖), 0.1998/0.0974/0.1756 →
  0.200/0.097/0.176, H_state 0.5043/0.3631/0.4670**. 4개 해협만 쓰면 128 units·0.2069/0.5106로 달라진다 —
  그래서 어떤 표도 그 조합을 쓰지 않는다는 문장을 넣었다.
- **Fig. 3 병합(2)**: 두 PNG를 패널군 **(a) H_state / (b) H_episode** 한 장으로 합쳤다(공유 컬러바).
  패널이 작아져 지역 풀네임이 축에서 잘리므로 **2글자 코드(KG·VS·SD·LS·BC)** + 캡션 범례로 바꿨다.
  `p10_figs.py`의 fig-3 블록도 같이 고쳐 재생성이 병합본을 만든다.
- **NISAR 문장(3)**: `stage4/REPILOT_RESULTS.md` §R2.2 확인 — 겹침 ≥50 %를 EPSG:3413에서 강제하면
  524 → **99건(고유 92쌍)**. 요청 문구를 쓰되 **dual-pol·6 h**라는 실제 제약을 빠뜨리지 않도록 함께 적었다.
- **highlight 2(4)**: 요청 문구는 **86자**로 한 자 초과 → 쉼표 하나를 빼 **정확히 85자**로 맞췄다.
- **[중요] 참고문헌 렌더링(5)**: pandoc·bibtexparser 둘 다 없어 파서/포매터를 직접 썼다.
  DOI 있는 71건은 **Crossref에서 저자 이니셜·권·페이지·저널명**을 받아 채우고, 없는 7건(책 2·회색문헌 2·
  arXiv 1 등)은 서지 문자열을 그대로 쓰고 **아무것도 만들지 않았다**. 제목은 검증본 그대로(휴리스틱
  sentence-case가 "NOAA→nOAA", "physics-Based"를 만들어서 폐기).
  **본문 인용 74건 전부 해소, 미해소 0, 중의 0.** 렌더링 중 **중복 키 2건 발견** —
  `howell2022motion`=`howell2022generating`, `aksenov2017future`=`aksenov2017navigability`(DOI·제목 동일,
  각각 목록에 두 번 찍혔을 것) → 제거해 **78 → 76**. 또 References 산문이 회색문헌을 "3건(CHNL 포함)"이라
  했는데 CHNL 항목은 서지에 없다 → **2건**으로 정정.
  **미인용 18건**은 보고만 했다: 서지가 인용 목록이 아니라 클러스터 검증 목표에 맞춰 만들어졌기 때문이며,
  Elsevier는 미인용 참고문헌을 거부한다. 인용할지 뺄지는 논문이 무엇을 주장할지의 편집 판단이라 손대지 않았다.
- **제목(6)**: "…: what Sentinel-1 could and could not see", working-title 주석 삭제.
- **graphical abstract(7)**: `figures/graphical_abstract.png` **정확히 1600×900**.
  스크립트가 픽셀 크기·모든 텍스트의 캔버스 내부 여부·**텍스트 쌍 겹침 없음**을 assert한다(3회 반복해 잡았다).
  viridis + 액센트 1색(#d95f02), 최소 8.5 pt. 기간 평균은 스크립트가 다시 계산해 **0.200/0.097/0.176**.
- **Zenodo 폴더(8)**: `paper/zenodo/` **68파일 7.41 MiB** — GeoTIFF 18(25 km, EPSG:3413, float32, NaN nodata),
  stage5 표 41, 코드 9, sha256 매니페스트, README. **업로드하지 않았다.** 인용 블록의 저자란은
  `[AUTHOR]`로 비워 뒀다 — git 아이덴티티(`yjchoi83`)는 검증된 인명이 아니고, 틀린 저자명은 빈칸보다 나쁘다.
- **분량**: 초록 **249**, 본문 **9,306**(표·캡션·후미 제외), 헤딩 포함 **9,497** — 둘 다 9,500 이하.
  item 1이 본문을 +110 늘려 헤딩 포함 수치가 9,501이 되었으므로, **값을 하나도 바꾸지 않고**
  중복 진술 5곳(§4.1 그룹 설명→§2.1 포인터, §3.1 부트스트랩→§6.7 포인터, §5.2 3위성→§4.4 포인터,
  §6.5 마무리 반복 삭제, §4.6·§4.3 문장 압축)을 줄여 되돌렸다.
