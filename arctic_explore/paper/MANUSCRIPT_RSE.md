...
Arctic navigation support rests on synthetic aperture radar, yet observing capability is reported as scene counts or revisit intervals — neither says whether a hazard is caught. We define a hazard-timescale observability metric H: the probability that a randomly occurring convergence event overlaps at least one acquisition, given the length-biased acquisition-gap distribution and an independently measured hazard-duration distribution. Durations come from 82,439 convergence events in 2016–2025 International Arctic Buoy Programme pairs; gaps from Sentinel-1 metadata for 1,771 cells of 25 km in sixteen Arctic regions, 2016–2026. At Northern Sea Route chokepoints, H for episode-scale hazards was 0.200 before Sentinel-1B was lost and 0.097 in 2022–2024. Taking H ≥ 0.8 as the requirement, none of 512 region-season-year units reached it for episode-scale hazards and no chokepoint unit on either metric, so the requirement was unmet before the gap. A remove-one-platform experiment reproduces the 2022–2024 collapse within 10 percentage points in fifteen of sixteen regions: that period is explained by satellite count. The incomplete recovery is not: scene volume returned to 88.5 % with three platforms, yet European Space Agency acquisition plans allocate 87.1 % of pre-loss coverage to the European–Russian sector against 123.4 % to North America, a deficit that survives control for Sentinel-1C ramp-up. Availability of the Copernicus DTU Sentinel-1 drift product tracks H across region-years (Spearman 0.711). A second platform is worth 0.10–0.14 of H, leaving the chokepoints far below requirement at every count observed, though twenty image pairs show their convergence is retrievable in winter and freeze-up.
...
A second gap concerns causal attribution. The failure of Sentinel-1B in December 2021 and the subsequent entry of Sentinel-1C and Sentinel-1D have been treated informally as a natural experiment on observing capability. That the sampling of an observing system propagates into the drift and deformation derived from it has been quantified for temporal and spatial resolution (Geiger and Drinkwater, 2001) and, for gaps in a Lagrangian ice-floe network, addressed by interpolation (Covington et al., 2022); and decade-scale records built from the Sentinel-1 archive now span the 2021 discontinuity itself (Wulf et al., 2024; Wuite et al., 2026; Wulf et al., 2026). Those records are presented as product climatologies, however, and do not quantify what the interruption did to their own availability. Nor have the loss period and the recovery period been separated, and they need not share a mechanism: a shortfall caused by having fewer satellites and a shortfall caused by where the remaining capacity is pointed have different remedies and are distinguishable only if the acquisition plans, not merely the acquired archive, are examined. A third gap is that the open Arctic SAR record depends on a single mission, so the propagation of an interruption into the products navigators use is both plausible and, as far as we are aware, unquantified.
...
**Table 1.** Regions, cell counts and role. Cells are 25 km on EPSG:3413. *The chokepoint group used in all chokepoint statistics comprises the four NSR straits and the Bering–Chukchi approach (five regions; 160 main-season units), that is every row marked "NSR chokepoint" or "approach".*
...
**Figure 1.** Construction of the observability metric H. **(a)** A sequence of acquisitions defines gaps g_i. The two hazards drawn have **the same duration** and differ only in when they begin: the filled one straddles an acquisition and is caught, the open one begins and ends inside g₅ and is missed. **A hazard is missed only if it both begins and ends inside a single gap**, which is why H depends on the distribution of gap lengths and not on their mean. **(b)** H as a function of D for three single fixed gaps and for the length-biased mixture of the gaps in panel (a), with the dashed line marking the H = 0.8 requirement; the mixture is controlled by the longest gaps at small D. `fig01_H_schematic.png`
...
Cell-level values are aggregated to region, season and year by unweighted averaging over cells, and uncertainty is expressed by a block bootstrap over cells with 1,000 draws. We report those intervals but do not interpret them as sampling uncertainty (Section 6.7).
...
**Figure 2.** Hazard timescales measured from buoy pairs, independent of Sentinel-1. **(a)** Cumulative distribution of convergence episode duration by magnitude class, from the one-hourly buoy subset; the dashed line marks the 12 h episode requirement. Because an event must span at least two sampling intervals, these durations are **left-censored at 2 h under the one-hourly sampling**, drawn as the dotted floor; the 1–3 km class is the class this bites, with 54.8 % of its events sitting on the floor and a median of exactly 2 h, so for that class the median is a property of the sampling and not of the ice. **(b)** Cumulative distribution of hazard-state persistence for events whose magnitude is at least 10 % of the pre-event separation; the dashed line marks the 24 h state requirement. `fig02_hazard_timescales.png`
...
Twenty Extra Wide ground-range-detected pairs were selected at Vilkitsky, Sannikov and Long Strait for freeze-up and winter of 2019–2021. Selection was balanced across the six region-season combinations by round-robin rather than by taking the globally best-overlapping pairs, which would have concentrated the sample at one strait.

Each scene was read from the HH band, converted to a relative decibel scale after division by the per-column median to suppress the across-track gradient, and reprojected from its ground-control points onto a common 200 m EPSG:3413 grid. Ice motion was retrieved with a two-stage scheme following the design of Korosov and Rampal (2017), whose combination of feature tracking with pattern matching is the standard formulation for this problem (see also Muckenhuber and Sandven, 2017; Demchev et al., 2017): oriented FAST and rotated BRIEF features matched on a decimated image pair give a coarse displacement field, which then initialises a normalised cross-correlation search on a 5 km node grid. **All pair-selection, matcher, node-acceptance and event-definition parameters are listed in Supplementary Table S1**; they are those used earlier in this work for a Chukchi melt-versus-winter comparison and were not retuned per season or per strait. The scheme is a reimplementation of the published two-stage design and is **not claimed to be identical to the `sea_ice_drift` package**, which is not distributed through the Python Package Index.

The success rate is the number of accepted nodes divided by the number at which a match was attempted, under the validity conditions of Table S1. Accepted displacements were converted to velocities, and divergence formed by centred differences on the node grid.

Candidate convergence events were connected components of the region where divergence falls below the tenth percentile of that pair's own divergence distribution, retaining components of at least 100 km². The definition is relative to each pair, so it identifies the most convergent tenth of each field rather than an absolute strain threshold, and its magnitude scale is set by the square root of component area, which is the origin of the baseline mismatch treated in Section 3.3. Every candidate event was subjected to the edge-artifact audit reported in Appendix A.

Virtual buoy pairs were formed on the same accepted-node fields: node pairs were drawn at random and retained when their separation fell in the 20–100 km band used for the real buoys, giving 240,949 pairs across the twenty windows. The change in separation over the acquisition interval was computed from the retrieved displacements and the implied rate compared with the buoy-derived thresholds of Section 3.2. This measures the SAR fields on the buoy baseline and, with the buoy rate threshold applied, selects them in the same way, which is what makes the two magnitude distributions comparable.

## 4. Results

### 4.1 The requirement was never met at the chokepoints

Taking the buoy record at face value, an episode-scale hazard of at least 3 km lasts a median of 8–12 h depending on season, 9 h pooled, and the resulting hazard state persists for a median of 24 h, which implies revisit requirements of 12 h and 24 h respectively if a large majority of hazards is to be bracketed. Section 3.2 turns those distributions into the criterion applied here: a region-season-year unit **meets the requirement when H ≥ 0.8**, that is when at least four fifths of hazards of the stated class are caught. On that criterion the requirement was never met at the chokepoints, and for episode-scale hazards it was never met anywhere. Of the 512 region-season-year units falling in the three main seasons, **none reached H_episode ≥ 0.8** for the ≥ 3 km class; the maximum over the whole record is 0.640 and the maximum at the chokepoints 0.467. The longer-lived hazard state does reach the criterion, but only away from the shipping lanes: **25 of the 512 units reached H_state ≥ 0.8**, all of them in four large high-latitude regions — Greenland Sea / Fram 13 units, Baffin Bay 5, Barents / Svalbard 4, Lancaster Sound 3 — and **none of the 160 chokepoint units** reached it, the chokepoint maximum being 0.793 at Kara Gate. Here and throughout, "the chokepoints" is the five-region group of Section 2.1.

Reported conventionally the picture is the same, which shows the conclusion does not rest on the buoy-derived durations. The hazard-agnostic coverage O(12 h) of Section 3.1 reached 0.5 in no unit at all; its maximum is 0.278 across the three main seasons and 0.320 over all 688 units for which O(Δt) is defined, the difference being a single shoulder-season unit in Baffin Bay. The statement that the 24 h criterion is reached in four regions refers specifically to **O(24 h) ≥ 0.8**, which is met in 38 of the 512 main-season units, all of them in the four large high-latitude regions just named and in none of the Northern Sea Route chokepoints in any year of the record, the highest chokepoint value being 0.475. O(Δt) is carried only for this comparison with published revisit statistics; every requirement statement in this paper is made on H.

The period means say the same thing. Before the loss of Sentinel-1B, chokepoint H_episode for the 3 km class stood at 0.200 and H_state at 0.504 (Fig. 3). Four in five episode-scale convergence events at the chokepoints were therefore already going unseen in 2019–2021, a period with two satellites operating normally. This is the central negative result of the paper, and it is not a consequence of the constellation gap. *Caveat: the 12 h and 24 h intervals and the 0.8 criterion alike are derived from the buoy duration distribution rather than from an operator's stated need, so they express what would be needed to catch the measured hazards, not a service specification agreed with an ice service.*

**Figure 3.** Observability across the study domain, for the ≥ 3 km magnitude class. **(a)** The sixteen regions on EPSG:3413 with Natural Earth 50 m coastline and land fill and a 30°/5° graticule; the five-region chokepoint group of Section 2.1 is filled and labelled in red, the Alaskan corridor sectors are dashed because they overlap Bering–Chukchi, and the remaining five are contrast regions. **(b)** H_episode per 25 km cell by season (rows) and period (columns), on the same coastline, with the chokepoint group outlined. **(c)** The chokepoint group at large scale, one row per region and one column per period, seasons pooled; the number printed in each panel is that region's mean for the period, formed as in Table 3. Colour is viridis from 0 to 1 throughout, where 1 means every hazard is caught, and the 0.8 requirement is marked on the scale. The corresponding H_state maps are Supplementary Figure S1. `fig03a_overview.png`, `fig03b_H_episode_maps.png`, `fig03c_chokepoint_strip.png`


### 4.2 The 2022–2024 gap widened an existing deficit

Between 2022 and 2024 chokepoint H_episode fell from 0.200 to 0.097 and H_state from 0.504 to 0.363; across all sixteen regions H_episode fell from 0.267 to 0.151 and H_state from 0.562 to 0.441 (Fig. 3, Fig. 4, Table 3). Figure 4 resolves the same quantities region by region and season by season. At the chokepoints this means that roughly nine in ten episode-scale hazards went unobserved during the gap, against four in five before it.

**Figure 4.** H by region and season-period, values printed in each cell. Rows are the sixteen regions, grouped by sector, with the chokepoint-group regions of Section 2.1 in bold red; columns are the three seasons crossed with the three periods. **(a)** H_state; **(b)** H_episode, both for the ≥ 3 km class. Colour is viridis from 0 to 1 and leading zeros are dropped in the printed values. `fig04_H_heatmap.png`


**Table 3.** Observability by period, for the ≥ 3 km magnitude class. Cell-level means, unweighted over cells; the three main seasons only. "NSR chokepoints" is the five-region group of Section 2.1.
...
**Table 3b.** Observability by magnitude class at the chokepoints, H_episode. Durations from the one-hourly buoy subset. Aggregation is the same two-stage unweighted mean used in Table 3 — cells to region-season-year, then over the 45 pre, 45 during and 25 post units of the five-region chokepoint group of Section 2.1 — so the ≥ 3 km row reproduces the chokepoint column of Table 3 exactly. The period counts are 5 regions × 3 seasons × 3 years for pre and during, and 5 × 3 for 2025 plus 5 × 2 for 2026, whose freeze-up falls outside the record.

| Class | pre 2019–21 | during 2022–24 | post 2025–26 |
|---|---|---|---|
| ≥ 1 km | 0.090 | 0.044 | 0.079 |
| 1–3 km | 0.063 | 0.030 | 0.056 |
| ≥ 3 km | 0.200 | 0.097 | 0.176 |
| ≥ 5 km | 0.257 | 0.127 | 0.228 |
...
Expressing those fields in the buoy-pair metric closes the loop. Sampling 240,949 virtual buoy pairs at 20–100 km separation on the twenty fields, and applying the buoy rate threshold so that the selection matches as well as the measurement, 0.50 % of pairs qualify and their median closure is 5.88 km, with a ninetieth percentile of 6.94 km and a maximum of 10.63 km (Fig. 5). Strait convergence of the magnitude the buoy record describes is therefore present in the SAR fields, concentrated at Vilkitsky in winter where 1.86 % of virtual pairs qualify. *Caveat: the nineteen individual events detected by the connected-component procedure are unvalidated, since no buoy lay within 100 km of any of them; they are reported in Appendix A and no inference rests on them.*

**Figure 5.** Strait drift fields expressed in the buoy-pair metric. Exceedance of closure over the acquisition window for 240,949 virtual buoy pairs at 20–100 km separation, unconditionally and after applying the buoy rate threshold, on logarithmic axes; the y axis is the fraction of **all** pairs, so the two curves are directly comparable. The 3 km and 5 km class boundaries are marked separately, dashed and dotted. Annotated are the median closure of the threshold-passing pairs, 5.88 km, and the 0.50 % of all pairs that pass. The inset shows the negative side of the same distribution, opening rather than closure, on identical axes. `fig05_virtual_pairs.png`


Four of the retrieved divergence fields are shown in Fig. 6.

**Figure 6.** Retrieved divergence fields at two straits, for four of the nineteen candidate convergence components of Appendix A: E002 and E001 at Vilkitsky on 5 January 2019, 125 km² each, and E011 and E010 at Sannikov on 13 January 2019, 100 and 400 km². Each panel is **cropped to the two-scene footprint overlap**, whose boundary is the orange dashed line; nothing outside that overlap is drawn, because outside it one image contributes no data. Grey shade is the after-scene Extra Wide HH; over it the divergence retrieved by the Section 3.6 matcher is drawn as a semi-transparent raster on one 5 km node per block, on a **common diverging scale of ± 0.10 d⁻¹** with negative, that is convergent, in red. The delineated component is the connected region below the tenth percentile of that pair's own divergence distribution, a relative and not an absolute strain threshold. Each panel carries a 50 km scale bar and a graticule, and the before-scene appears as a thumbnail on the same crop. *Edge audit:* all four components pass the overlap-boundary test of Appendix A, with centroid distances to the boundary of 18.9, 76.7, 46.0 and 32.4 km in panel order and no constituent node within 5 km of it, so none is a boundary artifact. The components are unvalidated against in-situ data and no main-text result depends on them. `fig06_strait_examples.png`
...
The acquisition plans show the same asymmetry before any acquisition takes place. Counting planned segments that cover at least half a region box, the European–Russian sector is planned at 87.1 % of its 2019–2021 rate against 123.4 % for North America, with Barents planned at 56.8 % (Fig. 8, Table 5). Because this appears in the plan rather than only in the outcome, the shortfall is an allocation decision and not a consequence of constellation size. Retrieving the Sentinel-1A plans for the gap years themselves shows that the reallocation did not begin in 2025: during 2022–2024, when a single platform had to be shared across the Arctic, the European–Russian sector was planned at 49.8 % of its pre-loss rate while the North American sector was planned at 62.0 %, so the sectoral asymmetry was already present when capacity was scarce and then widened once capacity was restored. The claim requires narrowing, however: within the same sector Vilkitsky is planned at 147.1 % and acquired at 156.1 %, and Long Strait at 102.9 %, so the deficit is specific to the Barents–Kara–Laptev sub-sector rather than to the European–Russian Arctic as a whole. *Caveat: acquisition-segment files describe planned datatakes and do not guarantee execution or downlink. The gap-year plans are Sentinel-1A only, because no second platform was flying, so their absolute level is not commensurable with the two-platform pre-loss and post-loss columns; only the ratio between sectors within the gap-year column is comparable, and it is that ratio, not the absolute level, that carries the claim.*
...
**Figure 8.** European Space Agency planned acquisition segments against acquired scenes, per region, on a logarithmic axis. Panels are the two sectors, named in the panel titles; chokepoint-group regions are in bold red. Five bars per region: planned per year for 2019–2021, for the gap years 2022–2024 and for 2025, then acquired per year before and after the gap. The gap-year bars are Sentinel-1A only, so their absolute level is not commensurable with the two-platform columns and only the ratio between sectors is comparable, as Table 5 sets out. `fig08_planned_vs_acquired.png`
...
Three separate arguments converge on the conclusion that the chokepoint deficit is not a procurement problem. The first is the design curve. Adding a second platform buys 0.10 to 0.14 of H depending on season, which leaves the chokepoints at 0.386 to 0.586 against a requirement of 0.8; closing that distance would require roughly two further doublings of the acquisition rate, which no announced constellation provides. The third platform cannot yet be assessed (Section 4.4). The second is the magnitude structure of the hazard itself. Small convergence events are both more common and shorter: for the 1–3 km class the median episode duration is 2–4 h, and chokepoint H for that class during the gap is 0.030, meaning that 97 % of such events pass unobserved. Catching them would require a revisit of a few hours, which is three to six times more demanding than the 12 h figure quoted as the episode requirement, and that figure is itself the relaxed value appropriate to the ≥3 km class.

The third argument is that observation is not the only bottleneck. Multiplying H by the retrieval success rate measured on real image pairs gives an effective observability at the chokepoints of 0.236–0.345 in winter and 0.171–0.238 in freeze-up across the three periods, but only 0.006–0.008 in the melt season (Table 7), because classical C-band feature tracking largely fails once the surface begins to melt. In that season no increase in acquisition frequency can raise the effective value above about 0.01, and the limiting factor is the retrieval algorithm and the radar band rather than the observing cadence. The freeze-up rate, previously unmeasured, is 0.392 pooled over the ten freeze-up pairs, so a working assumption of 0.47 carried over from winter was about 19 % optimistic. A recent benchmark of forty-eight deep-learning optical-flow models on RADARSAT-2 ScanSAR imagery reports endpoint errors of 300–400 m against GNSS-tracked buoys and might change this picture (Martin and Gallego, 2025), but it was evaluated on fifty-four scenes from a single March-to-May campaign in the Beaufort Sea, so it cannot be assumed to transfer to melt or freeze-up conditions; the melt-season and incidence-angle limits of C-band are themselves well documented (Park et al., 2018; Lohse et al., 2020; Singha et al., 2021).
...
No open, free, systematically acquired SAR over the Arctic marine domain matches Sentinel-1's coverage, which is why an interruption to one mission propagates as directly as Section 4.6 shows. The three sources offered as mitigation each change something different. The RADARSAT Constellation Mission would improve the North American sectors (Dabboor and Geldsetzer, 2014; Howell et al., 2022), but is not openly distributed on Sentinel-1's terms and does not prioritise the Russian Arctic chokepoints that dominate our deficit. NISAR adds L-band (Meyer et al., 2011; Singha et al., 2021), which penetrates a melting surface far better than C-band and is the one development that could attack the melt-season retrieval failure rather than the cadence; our audit of its first season found only 92 NISAR–Sentinel-1 pairs with at least half-footprint overlap, all below 78° N, counting dual-polarisation frames within six hours of each other, so its near-term contribution is to physics, not coverage. Commercial constellations can be tasked onto a strait, which suits incident response, but that is not the systematic background coverage a climatology of observability requires. None of the three removes the Section 4.4 finding that a further platform is worth 0.10 to 0.14 of H; they change the deficit's composition, not its size. We make no procurement recommendation.
...
**6.3 Shared-archive dependence.** The product-consequence result of Section 4.6 relates the availability of the DTU drift product to H, but the DTU product is generated from Sentinel-1. The correlation is thus between two functions of one archive. The direction of causation is not in doubt, since acquisition precedes retrieval, but the result cannot be described as independent verification, and we do not claim that a Sentinel-1-independent dataset confirms the propagation. None was available: the OSI SAF drift products are passive-microwave and scatterometer based and no Sentinel-1-based OSI SAF drift product exists, and the Norwegian ice-chart archive is issued on a fixed weekday schedule whose issuance count was unchanged through the gap — 248, 253, 251 and 250 issuance days in 2019, 2021, 2023 and 2025 respectively — so it cannot register a propagation signal even in principle.

**6.4 Planned segments versus acquired scenes.** The acquisition-segment archives describe planned datatakes. A planned segment may fail to execute, may not downlink, or may be superseded, and the archives give no means of distinguishing these outcomes. The plan-versus-acquisition comparison of Section 4.5 is therefore a comparison of intent with outcome, not an audit of execution. The gap-year archives that are available are Sentinel-1A only, since no second platform was flying, so their absolute planned volume cannot be set against the two-platform pre-loss and post-loss figures; only the ratio between sectors within the gap years is comparable across periods, and the statement about tasking during the gap rests on that sector ratio alone.

**6.5 Sentinel-1D ramp-up.** Sentinel-1D first appears in the archive in April 2026 with six months of data and no established plateau, so its routine-operations onset could not be determined by the criterion applied to Sentinel-1C, which reached routine operations in April 2025. The three-platform window consequently spans April to August 2026 only, comprising melt-season months, and does not represent a mature three-satellite constellation in freeze-up or winter.

**6.6 H is an upper bound.** A single acquisition falling inside a hazard window is counted as an observation, but retrieving ice motion requires an image pair, and inspection of the strait record shows that the two conditions are far from equivalent: at Sannikov, convergence days with an acquisition within 24 h exist, whereas pairs separated by 24 h or less largely do not. The effective values obtained by multiplying H by measured retrieval success — 0.236–0.345 in winter, 0.171–0.238 in freeze-up and 0.006–0.008 in melt at the chokepoints, as tabulated in Table 7 — are the operationally meaningful figures, and they are themselves upper bounds because the retrieval rates come from a small number of scenes in limited conditions.
...
**6.8 Small events require shorter revisit than the headline requirement.** The 12 h episode requirement is the value appropriate to convergence of at least 3 km. For the 1–3 km class the median episode duration is 2–4 h and chokepoint H falls to 0.030 during the gap. Any operational specification derived from this paper must therefore state its magnitude class explicitly, because the revisit implied by the requirement varies by a factor of three to six across the classes we can measure, and the smallest class we can measure is not the smallest class that exists.

**6.9 Additional constraints of scope.** Regions are approximated by longitude-latitude boxes rather than by strait polygons, and the coverage criterion of half a region box is sensitive to box size. The Alaskan corridor sectors overlap the Bering–Chukchi region, so the two are not independent in any aggregate. The novelty assessment underpinning the framing of this work, originally conducted with Crossref and arXiv only because the Semantic Scholar interface returned rate-limit responses, has been repeated with an authenticated search of twenty-one queries across the three framing claims. No indexed work computes the probability that a hazard is caught from an acquisition-gap distribution, quantifies the consequences of the Sentinel-1B loss, or audits regional allocation in the published acquisition plans, so the framing stands; a negative search, however, bounds only what an index contains.
...
Third, the incomplete recovery of 2025–2026 has a different cause. With three platforms the aggregate scene rate is 88.5 % of its pre-loss value, but the European Space Agency acquisition plans allocate 87.1 % of pre-loss coverage to the European–Russian sector against 123.4 % to North America, and the deficit survives control for Sentinel-1C ramp-up. The shortfall is an allocation decision, specific to the Barents–Kara–Laptev sub-sector, not a consequence of constellation size.
