...
P13 added words to the main text for item 1 (the chokepoint-group definition in §2.1 and §4.1, +110) and to
the captions and back matter (Figure 3's merged caption, the Table 1 and Table 3/3b group notes, the Data
availability deposit sentence). To hold the heading-inclusive measure under 9,500, five passages that
restated something already stated elsewhere were compressed, with **no value altered**: the §4.1 group
gloss became a pointer to §2.1; the §3.1 bootstrap caveat became a pointer to §6.7; the §5.2 third-platform
sentence became a pointer to §4.4; §6.5 dropped its closing restatement of the Fig. 6 caption; and the §4.6
within-region correlation sentence and the §4.3 retrieval sentence were tightened, both keeping their numbers
(0.727 and the twenty-pair result).

§3.6's matcher, node-acceptance, pair-selection and event-definition parameters live in
`paper/SUPPLEMENTARY.md` (Supplementary Table S1); values are unchanged, only relocated.
...
## [P13-5] Rendered reference list

`paper/MANUSCRIPT_RSE.md` is generated by `scratch/P13/make_rse.py` and differs from `MANUSCRIPT.md` only in
carrying a rendered Elsevier Harvard author-year list in place of the References prose. **`MANUSCRIPT.md`
remains the source of record.** No pandoc and no bibtexparser exist in this environment, so `scratch/P13/bib.py`
parses the bib and `scratch/P13/render_refs.py` formats it. Author initials, volume, pagination and container
titles come from Crossref for the 71 DOI-bearing entries (`scratch/P13/crossref_meta.json`); where no Crossref
record exists — the two books, the two grey-literature items, the one arXiv preprint — the bibliography string
is used verbatim and nothing is invented. Titles are printed as verified, without heuristic sentence-casing.

| Check | Result |
|---|---|
| in-text citation instances | **74** |
| unresolved | **0** |
| ambiguous (first author + year matching more than one work) | **0** |
| distinct keys cited | 58 |
| **duplicate keys found and removed** | `howell2022motion` = `howell2022generating` (DOI 10.5194/tc-16-1125-2022); `aksenov2017future` = `aksenov2017navigability` (DOI 10.1016/j.marpol.2015.12.027) — identical DOIs and titles, each would have printed twice |
| bibliography entries | 78 → **76**, for 76 distinct works |
| entries in the bib but never cited in the text | **18** — reported, not altered; see below |

The 18 uncited entries are `aksenov2017navigability`, `bellona2025nsr`, `bhattacharjee2024estimation`,
`chen2024mmseaice`, `cheng2020accuracy`, `eriksson2025finnish`, `evans2023iceberg`, `geudtner2021copernicus`,
`huang2024deeplearning`, `lee2020sentinel`, `lensu2019bigdata`, `muckenhuber2016opensource`, `sumata2014inter`,
`stokholm2024autoice`, `taleghan2025icefmbench`, `torres2017evolution`, `vardi1989multiplicative`,
`zhao2023deeplearning`. They exist because the bibliography was assembled against per-cluster verification
targets rather than against the citation list. Elsevier rejects uncited references, so before submission each
must be either cited or dropped. That is an editorial decision about what the paper claims, not a consistency
fix, so nothing was added or deleted here.

Also corrected in the References prose: the grey-literature list said **three** items including "the CHNL
